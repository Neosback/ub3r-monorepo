package net.dodian.game.plugin.processor
...
class PluginModuleIndexSymbolProcessor(
    ...
    override fun process(resolver: Resolver): List<com.google.devtools.ksp.symbol.KSAnnotated> {
        ...
        val objectContents = discoverObjectsByInterface(allObjects, "net.dodian.game.content.objects.ObjectContent")
        val itemContents = discoverObjectsByInterface(allObjects, "net.dodian.game.content.items.ItemContent")
        ...
        val outputFile =
            codeGenerator.createNewFile(
                ...
                packageName = "net.dodian.game.plugin",
    ...
    private fun discoverInterfaceButtons(allObjects: List<Pair<KSFile, KSClassDeclaration>>): List<DiscoveredSymbol> {
        val interfaceButtonsType = "net.dodian.game.systems.ui.buttons.InterfaceButtonContent"
        return allObjects
            .filter { (file, declaration) ->
                file.packageName.asString().startsWith("net.dodian.game.content.interfaces") &&
    ...
    private fun discoverNpcModules(allObjects: List<Pair<KSFile, KSClassDeclaration>>): List<DiscoveredSymbol> {
        ...
        return allObjects
            .filter { (file, declaration) ->
                file.packageName.asString().startsWith("net.dodian.game.content.npcs.spawns") &&
    ...
    private fun discoverEventBootstraps(allObjects: List<Pair<KSFile, KSClassDeclaration>>): List<DiscoveredSymbol> {
        return allObjects
            .filter { (file, declaration) ->
                file.packageName.asString().startsWith("net.dodian.game.event.bootstrap") &&
    ...
    private fun buildOutput(
        ...
        out.appendLine("package net.dodian.game.plugin")
        out.appendLine()
        out.appendLine("import net.dodian.game.content.items.ItemContent")
        out.appendLine("import net.dodian.game.content.npcs.spawns.NpcContentDefinition")
        out.appendLine("import net.dodian.game.content.npcs.spawns.NpcModuleDefinitionBuilder")
        out.appendLine("import net.dodian.game.content.objects.ObjectContent")
        out.appendLine("import net.dodian.game.systems.ui.buttons.InterfaceButtonContent")
        out.appendLine()
