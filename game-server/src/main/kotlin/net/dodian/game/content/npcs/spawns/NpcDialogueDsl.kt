package net.dodian.game.content.npcs.spawns

import net.dodian.game.systems.ui.dialogue.DialogueEmote
import net.dodian.game.systems.ui.dialogue.DialogueFactory
import net.dodian.game.systems.ui.dialogue.DialogueOption
...
internal sealed interface NpcDialogueFrame {
    val label: String
}

internal data class NpcChatFrame(
...
internal data class PlayerChatFrame(
    override val label: String,
    val text: String,
    val emote: Int,
    val next: String?,
...
internal data class ActionFrame(
    override val label: String,
    val actions: List<NpcRuntimeAction>,
    val next: String?,
) : NpcDialogueFrame

internal data class ChoiceFrame(
    override val label: String,
    val title: String,
    val options: List<ChoiceOption>,
) : NpcDialogueFrame

internal data class EndFrame(
    override val label: String,
    val finishMode: NpcDialogueFinishMode,
...
class NpcChoiceBuilder {
    private val options = ArrayList<ChoiceOption>()

    fun option(
        text: String,
        goto: String? = null,
        finishMode: NpcDialogueFinishMode = NpcDialogueFinishMode.FINISH,
        closeInterfaces: Boolean = true,
        actions: NpcActionChainBuilder.() -> Unit = {},
    ) {
        val builtActions = NpcActionChainBuilder().apply(actions).build()
        options +=
            ChoiceOption(
                text = text,
                goto = goto,
    ...
    internal fun build(): List<ChoiceOption> = options.toList()
}

class NpcDialogueFlowBuilder {
    private val frames = LinkedHashMap<String, NpcDialogueFrame>()
    ...
    fun choice(
        ...
        init: NpcChoiceBuilder.() -> Unit,
    ) {
        val options = NpcChoiceBuilder().apply(init).build()
        require(options.size in 2..5) { "Dialogue choice '$label' must have 2..5 options." }
        register(ChoiceFrame(label = label, title = title, options = options))
    }

    fun action(
        label: String,
        next: String? = null,
        init: NpcActionChainBuilder.() -> Unit,
    ) {
        val actions = NpcActionChainBuilder().apply(init).build()
        register(ActionFrame(label = label, actions = actions, next = next))
    }
    ...
    internal fun build(): NpcDialogueFlow {
        val start = requireNotNull(startLabel) { "Dialogue flow has no frames." }
        return NpcDialogueFlow(start = start, frames = frames.toMap())
    }

    private fun register(frame: NpcDialogueFrame) {
        if (startLabel == null) {
            startLabel = frame.label
        }
        require(!frames.containsKey(frame.label)) { "Duplicate dialogue frame label '${frame.label}'." }
...
class NpcDialogueFlow internal constructor(
    ...
    private val frames: Map<String, NpcDialogueFrame>,
) {
    fun start(client: Client, npc: Npc) {
        DialogueService.start(client) {
            emit(label = start, npc = npc, path = LinkedHashSet())
        }
    }

    private fun DialogueFactory.emit(
        label: String,
        npc: Npc,
        path: LinkedHashSet<String>,
    ) {
        require(path.add(label)) { "Dialogue cycle detected at '$label'." }
        val frame = frames[label] ?: error("Unknown dialogue frame '$label'.")
        when (frame) {
            is NpcChatFrame -> {
                npcChat(npc.id, frame.emote, frame.text)
                val next = frame.next
                if (next != null) {
                    emit(next, npc, LinkedHashSet(path))
                } else {
                    finish()
                }
            }
            ...
            is PlayerChatFrame -> {
                playerChat(frame.emote, frame.text)
                val next = frame.next
                if (next != null) {
                    emit(next, npc, LinkedHashSet(path))
                } else {
                    ...
                    *frame.options
                        .map { option ->
                            DialogueOption(option.text) {
                                val next = option.goto
                                if (next != null) {
                                    option.actions.forEach { step ->
                                        action { client -> step(client, npc) }
                                    }
                                    emit(next, npc, LinkedHashSet(path))
                                } else {
                                    when (option.finishMode) {
                                        NpcDialogueFinishMode.FINISH ->
                                            finish(closeInterfaces = option.closeInterfaces) { client ->
                                                option.actions.forEach { step -> step(client, npc) }
                                            }

                                        NpcDialogueFinishMode.FINISH_THEN ->
                                            finishThen(closeInterfaces = option.closeInterfaces) { client ->
                                                option.actions.forEach { step -> step(client, npc) }
                                            }
                                    }
                                }
            ...
            is EndFrame -> {
                when (frame.finishMode) {
                    NpcDialogueFinishMode.FINISH ->
                        finish(closeInterfaces = frame.closeInterfaces) { client ->
                            for (step in frame.actions) {
                                step(client, npc)
                            }
                        }
                    ...
                    NpcDialogueFinishMode.FINISH_THEN ->
                        finishThen(closeInterfaces = frame.closeInterfaces) { client ->
                            for (step in frame.actions) {
                                step(client, npc)
                            }
                        }
...
class NpcConditionContext internal constructor(
    ...
    fun balloonsEventActive(): Boolean = Balloons.eventActive()
}
