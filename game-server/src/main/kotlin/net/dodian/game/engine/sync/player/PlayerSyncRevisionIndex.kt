    ...
    fun rebuild(activePlayers: List<Client>, tick: Long, activityIndex: PlayerChunkActivityIndex) {
        ...
        activePlayers.forEach { player ->
            ...
            if (state.lastChunkX != Int.MIN_VALUE) {
                if (moved || updated) {
                    activityIndex.bump(net.dodian.game.model.chunk.Chunk(state.lastChunkX, state.lastChunkY), previousLevel)
                }
            }
            if (moved || updated) {
        ...
        while (iterator.hasNext()) {
            ...
            if (state.lastChunkX != Int.MIN_VALUE) {
                activityIndex.bump(net.dodian.game.model.chunk.Chunk(state.lastChunkX, state.lastChunkY), state.lastLevel)
            }
            iterator.remove()
