...
import net.dodian.game.engine.sync.util.IntHashSet
import net.dodian.game.engine.sync.util.LongHashSet
...
import net.dodian.game.engine.lifecycle.PlayerLifecycleTickService
import net.dodian.game.systems.animation.PlayerAnimationService
import net.dodian.game.systems.combat.CombatRuntimeService
...
class EntityProcessor : Runnable {
    ...
    private fun processInboundPackets() {
        net.dodian.game.engine.metrics.InboundOpcodeProfiler.beginTick()
        val startNs = System.nanoTime()
        ...
        if (elapsedMs >= runtimePhaseWarnMs) {
            logger.warn(
                ...
                maxPendingBefore,
                maxPendingAfter,
                net.dodian.game.engine.metrics.InboundOpcodeProfiler.top3Summary(),
            )
        }
    }
    ...
    private fun processPlayer(
        ...
        if (startingHealth != player.currentHealth || startingPrayer != player.currentPrayer) {
            player.markSaveDirty(
                net.dodian.game.persistence.player.PlayerSaveSegment.STATS.mask or
                    net.dodian.game.persistence.player.PlayerSaveSegment.EFFECTS.mask or
                    net.dodian.game.persistence.player.PlayerSaveSegment.META.mask,
            )
        }
        if (startingX != player.position.x || startingY != player.position.y || startingZ != player.position.z) {
            player.markSaveDirty(net.dodian.game.persistence.player.PlayerSaveSegment.POSITION.mask)
        }
