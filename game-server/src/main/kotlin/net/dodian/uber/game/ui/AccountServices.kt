            ...
            DbAsyncRepository.withConnection { conn ->
                try {
                    ...
                    if (rs.next()) {
                        ...
                        if (hashedInput.equals(dbPass, ignoreCase = true)) {
                            // Correct! Now prompt for new password
                        ...
                        client.send(SendString("<col=ff0000>Failed to load account info.</col>", 36712))
                    }
                } catch (e: Exception) {
                    e.printStackTrace()
            ...
            DbAsyncRepository.withConnection { conn ->
                try {
                    ...
                    if (rs.next()) {
                        ...
                        val rows = psUpdate.executeUpdate()
                        if (rows > 0) {
                        ...
                        client.send(SendString("<col=ff0000>Failed to find salt.</col>", 36712))
                        client.send(SendString("", 36714))
                    }
                } catch (e: Exception) {
                    e.printStackTrace()
        ...
        try {
            DbAsyncRepository.withConnection { conn ->
                try {
                    ...
                    client.send(SendString(rankStr, 36708))
                    client.send(SendString("Created:", 36709))
                    client.send(SendString(createdStr, 36710))
