...
class GroundItemSpawnBuilder {
    private val spawns = ArrayList<GroundItemSpawn>()

    fun group(name: String, block: GroundItemSpawnGroupBuilder.() -> Unit) {
        GroundItemSpawnGroupBuilder(name, spawns).apply(block)
    }

    fun build(): List<GroundItemSpawn> = spawns.toList()
}

class GroundItemSpawnGroupBuilder internal constructor(
    @Suppress("unused")
    private val name: String,
    private val spawns: MutableList<GroundItemSpawn>,
) {
    fun spawn(x: Int, y: Int, z: Int, itemId: Int, amount: Int, displayTime: Int) {
        spawns += GroundItemSpawn(Position(x, y, z), itemId, amount, displayTime)
    }
}

private fun globalGroundSpawns(block: GroundItemSpawnBuilder.() -> Unit): List<GroundItemSpawn> {
    val builder = GroundItemSpawnBuilder()
    builder.block()
...
object GlobalGroundItemSpawns {
    @JvmField
    val spawns: List<GroundItemSpawn> =
        globalGroundSpawns {
            group("Troll Items") {
