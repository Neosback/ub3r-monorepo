package net.dodian.uber.game.skill.farming
...
import net.dodian.uber.game.skill.skillguide.SkillGuide
...
class Farming {
    ...
    fun Client.interactItemBin(objectId : Int, itemId: Int) : Boolean {
        ContentRuntimeApi.onFarmingCompostInteraction(this)
                    ...
                    if (patch.objectId[slot] == objectId) { //We got the correct objectId!
                        if(findPatch(objectId, 1) == FarmingData.patchState.WEED.toString() && findPatch(objectId, 3).toInt() < 3) {
                            ...
                            if (findPatch(objectId, 1) == FarmingData.patchState.GROWING.toString()) {
                                ...
                                updateFarmPatch()
                                markFarmingDirty()
                                ContentRuntimeApi.onFarmingPatchInteraction(this)
...
object FarmingSkillPlugin : SkillPlugin {
    override val definition =
        skillPlugin(name = "Farming", skill = Skill.FARMING) {
            ...
            objectClick(preset = PolicyPreset.GATHERING, option = 4, *guideObjects) { interaction ->
                val client = net.dodian.uber.game.engine.systems.skills.SkillEngineAccess.client(interaction.player)
                val objectId = interaction.objectId
                val position = interaction.position
                val obj = interaction.definition
            ...
            objectClick(preset = PolicyPreset.GATHERING, option = 5, *compostBinIds) { interaction ->
                val client = net.dodian.uber.game.engine.systems.skills.SkillEngineAccess.client(interaction.player)
                val objectId = interaction.objectId
                val position = interaction.position
                val obj = interaction.definition
            ...
            itemOnObject(preset = PolicyPreset.GATHERING, objectIds = compostBinIds) { interaction ->
                val client = net.dodian.uber.game.engine.systems.skills.SkillEngineAccess.client(interaction.player)
                val objectId = interaction.objectId
                val position = interaction.position
                val obj = interaction.definition
