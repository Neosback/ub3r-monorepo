package net.dodian.uber.game.world.cache;

import net.dodian.uber.game.world.cache.index.IndexTable;
...
public class Cache implements Closeable {
    ...
    public static void load() {
        try {
            singleton = new Cache(defaultCacheDirectory());
    ...
    public static File defaultCacheDirectory() {
        String envPath = System.getenv("MYSTIC_CACHE_DIR");
        if (envPath != null && !envPath.isBlank()) {
            return new File(envPath);
        }
        return new File("./data/cache/mystic");
    }
     ...
     * @throws net.dodian.uber.game.world.cache.InvalidCacheException if the cache is corrupt or invalid.
     */
