...
import net.dodian.uber.game.activity.partyroom.PartyRoomOfferState;
...
/**
 * Java implementation detail retained while the Player entity is migrated to Kotlin.
 * New gameplay state and behavior must live in named Kotlin services/state components,
 * not in this compatibility core.
 */
/**
 * Transitional Java implementation behind the public Kotlin {@link Player} entity.
 *
 * Kotlin's public Player subclass cannot extend a package-private JVM superclass.
 * Keep this visible only for the migration boundary; gameplay callers should use
 * Player and focused services rather than depend on this type.
 */
public abstract class PlayerCore extends Entity {
    private static final AtomicLong NEXT_SESSION_GENERATION = new AtomicLong(1L);
    private static final Logger logger = LoggerFactory.getLogger(PlayerCore.class);
    ...
    private final PlayerAccountState accountState = new PlayerAccountState(asPlayer());
    private final PlayerContentRuntimeState interactionState = new PlayerContentRuntimeState();
    private final PlayerQuestProgressState questProgressState = new PlayerQuestProgressState();
    private final PartyRoomOfferState partyRoomOfferState = new PartyRoomOfferState();
    private final BossKillLogState bossKillLogState = new BossKillLogState();

    public PlayerContentRuntimeState getContentRuntimeState() {
        return interactionState;
    }

    public RunePouchState getRunePouchState() {
        return runePouchState;
    }

    public PlayerQuestProgressState getQuestProgressState() {
        return questProgressState;
    }

    public PartyRoomOfferState getPartyRoomOfferState() {
        return partyRoomOfferState;
    }

    public BossKillLogState getBossKillLogState() {
        return bossKillLogState;
    }
    private final PlayerMovementState movementState = new PlayerMovementState(asPlayer());
    private final PlayerUpdateState updateState = new PlayerUpdateState(asPlayer());
    ...
    // Sized to bankSize() (= playerBankSize / 2), matching every other bank structure
    // (bankSlotTabs, bankContainerSlotMap in PlayerBankService) - the raw playerBankSize was
    // double what the bank system actually ever uses (nothing indexes these beyond bankSize(),
    // and every access site already bounds itself off .length, not a hardcoded constant).
    public int[] bankItems = new int[bankSize()];
    public int[] bankItemsN = new int[bankSize()];
    ...
    // Protocol cap on simultaneous local players a viewer can track (matches
    // StagedPlayerSynchronizationService.MAX_LOCALS) - not the total server player cap, so this
    // must stay a fixed wire-protocol constant, never sized off the configured player capacity.
    public final static int maxPlayerListSize = 255;
    public Player[] playerList = new Player[maxPlayerListSize];
    public int playerListSize = 0;
    public PlayerSlotMembershipSet playersUpdating = new PlayerSlotMembershipSet(DotEnvKt.getGameMaxPlayers() + 1);
    ...
    private final RouteDestination routeDestination = new RouteDestination();
    public int m4001 = 0;
    public int m4002 = 0;
    public int m4003 = 0;
    public int m4004 = 0;
    ...
    public int m4006 = 0;
    public int m4007 = 0;
    // Runecrafting owns pouch definitions and persisted contents.
    private final RunePouchState runePouchState = new RunePouchState();
    //Agility
    ...
    private final PlayerStats stats = new PlayerStats(asPlayer());
    private final PlayerAppearanceState appearanceState = new PlayerAppearanceState(asPlayer());
    private final PlayerProgressState progressState = new PlayerProgressState(asPlayer());
    private final PlayerCombatState combatState = new PlayerCombatState(asPlayer());

    protected PlayerCore(int slot) {
    ...
    /** The only concrete PlayerCore instances are Kotlin Player subclasses. */
    ...
    public boolean isWalkBlockedAtCycle(long cycle) {
        return cycle < walkBlockUntilCycle;
    }
    ...
    public long getWalkBlockUntilCycle() {
        return walkBlockUntilCycle;
    }
    ...
    public boolean withinDistance(Player otherPlr) {
        ...
        // Bound must match what the add-local wire encoding can represent: writeStagedLocalAdd
        // wraps a negative delta by +32 into an unsigned 5-bit field, which the client re-signs
        // as (raw > 15) ? raw - 32 : raw - i.e. only [-16, 15] round-trips correctly. A delta of
        // exactly +16 wraps to the same wire value as -16, aliasing the position. Matches the
        // (-16, 15) bound already used by withinDistance(Npc) below.
     ...
     */
    private long cachedNameAsLong = -1L;

    public String getPlayerName() {
        return accountState.getPlayerName();
    }

    public long getPlayerNameAsLong() {
        if (cachedNameAsLong == -1L) {
            String name = getPlayerName();
            cachedNameAsLong = name != null ? net.dodian.utilities.Utils.playerNameToInt64(name) : 0L;
        }
        return cachedNameAsLong;
    }
    ...
    public boolean isSynchronizationReady() {
        return synchronizationReady
                ...
                && !disconnected
                && getSlot() > 0
                && getSlot() <= DotEnvKt.getGameMaxPlayers()
                && getPosition() != null;
    }
    ...
    public void replaceMovementRoute(java.util.Collection<MovementPoint> waypoints, boolean running) {
        movementState.replaceRoute(waypoints, running);
    }

    public boolean hasMovementRoute() {
        return routeDestination.isNotEmpty();
    }

    public int movementRouteSize() {
        return routeDestination.size();
    }

    public MovementPoint movementDestination() {
        return routeDestination.destination();
    }

    public java.util.List<MovementPoint> movementRouteSnapshot() {
        return routeDestination.snapshot();
    }
     ...
     */
    private final long[] appearanceTicketsSeen = new long[DotEnvKt.getGameMaxPlayers() + 1];
    ...
    public SkillingEventState getSkillingEventState() {
        ...
        if (state == null) {
            ...
            state = new SkillingEventState(randomed, randomSkillId, chestEvent, chestEventOccur, false);
    ...
    public net.dodian.uber.game.engine.sync.player.PlayerSyncInvariantValidator.ViewerLocalSnapshot getPreviousSyncSnapshot() {
        return interactionState.getPreviousSyncSnapshot();
    }

    public void setPreviousSyncSnapshot(net.dodian.uber.game.engine.sync.player.PlayerSyncInvariantValidator.ViewerLocalSnapshot previousSyncSnapshot) {
        interactionState.setPreviousSyncSnapshot(previousSyncSnapshot);
    }
    ...
    PrayerManager prayerManager = new PrayerManager(asPlayer());
    ...
    public boolean blackMaskEffect(int npcId) {
        return net.dodian.uber.game.engine.systems.skills.SlayerCombatBridge.damageBonus((Client) this, npcId, false) == 1;
    }
    public boolean blackMaskImbueEffect(int npcId) {
        return net.dodian.uber.game.engine.systems.skills.SlayerCombatBridge.damageBonus((Client) this, npcId, true) == 2;
    }
    ...
    public boolean gotSlayerHelmet(Client c) {
        return net.dodian.uber.game.engine.systems.skills.SlayerCombatBridge.hasSlayerHelmet(c);
    }
    ...
    public void examineObject(Client c, int objectId, Position objPos) {
        ...
        net.dodian.uber.skills.farming.FarmingModule.INSTANCE.examineBin(
            net.dodian.uber.game.engine.systems.skills.SkillPlayerBridge.of(c), objectId);
        ...
        if(objectId == 378 && objPos.getX() == 2593 && objPos.getY() == 3108 && objPos.getZ() == 1) { //Check timer on a object!
            net.dodian.uber.game.model.objects.WorldObject globalObject =
                    GlobalObject.getGlobalObject(objPos.getX(), objPos.getY(), objPos.getZ());
            Object attachment = globalObject == null ? null : globalObject.getAttachment();
            if (attachment instanceof Long) {
                long timeLeft = (Long) attachment;
    ...
    public boolean rejectTeleport() {
        return net.dodian.uber.skills.thieving.ThievingModule.hindersTeleport(
                net.dodian.uber.game.engine.systems.skills.SkillPlayerBridge.of((Client) this));
    }
    public void loginPosition(int x, int y, int z) {
        moveTo(x, y, z);
        if(getPositionName(getPosition()) == positions.PYRAMID_PLUNDER)
            net.dodian.uber.skills.thieving.ThievingModule.resetPlunder(
                    net.dodian.uber.game.engine.systems.skills.SkillPlayerBridge.of((Client) this));
    }
    ...
    public void resetTabs() {
       ...
       // c.setSidebarInterface(7, 37128); // clan chat tab
        ...
        c.setSidebarInterface(11, 50020); // wrench tab - complete settings (fullscreen, zoom, key bindings, etc.)
    ...
    public Chunk getCurrentChunk() {
        return currentChunkX == -1 || currentChunkY == -1 ? null : Chunk.of(currentChunkX, currentChunkY);
    }
