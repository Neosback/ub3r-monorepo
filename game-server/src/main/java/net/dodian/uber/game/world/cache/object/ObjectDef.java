...
import net.dodian.uber.game.world.cache.MemoryArchive;
import net.dodian.uber.game.world.cache.util.ByteStream;
import net.dodian.uber.game.world.cache.util.ByteStreamExt;
...
public final class ObjectDef {
    ...
    public static void loadConfig() {
        try {
            Cache cacheSingleton = Cache.getSingleton();
            if (cacheSingleton == null) {
                throw new IllegalStateException("Mystic cache is not initialized.");
            }
            Archive configArchive = new Archive(cacheSingleton.getFile(0, 2));
            byte[] locDat = configArchive.getFile("loc.dat");
            byte[] locIdx = configArchive.getFile("loc.idx");
            if (locDat == null || locIdx == null) {
                throw new IllegalStateException("loc.dat/loc.idx missing in mystic cache config archive.");
            }
            archive = new MemoryArchive(new ByteStream(locDat), new ByteStream(locIdx));
        } catch (Exception e) {
            throw new IllegalStateException("Unable to load object definitions from mystic cache.", e);
        }
