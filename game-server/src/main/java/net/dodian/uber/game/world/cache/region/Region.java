package net.dodian.uber.game.world.cache.region;

import net.dodian.uber.game.world.cache.Archive;
import net.dodian.uber.game.world.cache.Cache;
import net.dodian.uber.game.world.cache.object.GameObjectData;
import net.dodian.uber.game.world.cache.util.ByteStream;
...
public class Region {
    ...
    private void addClip(int x, int y, int height, int shift) {
        try {
            ...
            CollisionService.addFlag(x, y, height, shift);
        } catch (Exception e) {
    ...
    private void removeClip(int x, int y, int height, int shift) {
        ...
        CollisionService.removeFlag(x, y, height, shift);
    }
    ...
    public static void load() {
        try {
            ...
            if (cache == null) {
                throw new IllegalStateException("Mystic cache is not initialized.");
            }
            Archive versionListArchive = new Archive(cache.getFile(0, 5));
            byte[] buffer = versionListArchive.getFile("map_index");
            if (buffer == null || buffer.length == 0) {
                throw new IllegalStateException("map_index not found in mystic cache.");
            }
            ...
            for (int i = 0; i < size; i++) {
                byte[] objectCompressed = null;
                byte[] groundCompressed = null;
                try {
                    if (mapObjectsFileIds[i] >= 0 && mapObjectsFileIds[i] <= cache.getFileCount(4)) {
                        objectCompressed = cache.getFile(4, mapObjectsFileIds[i]).getBytes();
                    }
                    if (mapGroundFileIds[i] >= 0 && mapGroundFileIds[i] <= cache.getFileCount(4)) {
                        groundCompressed = cache.getFile(4, mapGroundFileIds[i]).getBytes();
                    }
                } catch (Exception ignored) {
                }
                byte[] file1 = inflateBuffer(objectCompressed);
                byte[] file2 = inflateBuffer(groundCompressed);
        ...
        for (int level = 0; level < 4; level++) {
            for (int localX = 0; localX < 64; localX++) {
                for (int localY = 0; localY < 64; localY++) {
                    CollisionService.setFlag(absX + localX, absY + localY, level, 0);
                }
            }
        }
    ...
    public static byte[] inflateBuffer(byte[] compressed) throws Exception {
