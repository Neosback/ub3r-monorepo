...
public final class Player extends Entity implements RSPlayer {
    ...
    public void saveProfile(Client client) {
        ...
        // resetLogout() (reached from dropClient()'s failed-reconnect path) used to call this
        // unconditionally - with only the REMEMBER_ME check above, a failed login attempt could
        // still get persisted as a "saved account" since myUsername/myPassword hold whatever was
        // last typed, authenticated or not. Only actually-logged-in sessions should ever save.
        if (!Client.loggedIn) {
            return;
        }
        int[] skills = new int[23];
        if (client.maxStats != null && client.maxStats.length >= 23) {
            System.arraycopy(client.maxStats, 0, skills, 0, 23);
        } else {
            java.util.Arrays.fill(skills, 1);
            skills[3] = 10;
        }
        long lastLogin = System.currentTimeMillis();
        Profile profile = new Profile(client.myUsername, client.myPassword, gender, appearanceModels, appearanceColors, skills, lastLogin);
        ProfileManager.add(profile);
        refreshProfilePreview(profile);
    }

    /**
     * The live player model has already resolved every worn identity-kit and item model.
     * Use it for account previews so a cache miss cannot silently drop a helm, head, or cape.
     */
    public void refreshProfilePreview(Profile profile) {
        Client activeClient = Client.instance;
        if (activeClient == null || !activeClient.isClientThread()) {
            return;
        }
        ProfileManager.saveBodyPreview(profile, getAnimatedModel());
    }
