...
public class Profile {
    ...
    private static final int[] DEFAULT_SKILL_LEVELS;

    static {
        DEFAULT_SKILL_LEVELS = new int[23];
        java.util.Arrays.fill(DEFAULT_SKILL_LEVELS, 1);
        DEFAULT_SKILL_LEVELS[3] = 10; // hitpoints starts at 10
    }
    ...
    public Profile() {
        ...
        this.skillLevels = DEFAULT_SKILL_LEVELS;
        this.lastLogin = 0L;
    }

    public Profile(String username, String password, int gender, int[] equipment, int[] recolours) {
        this(username, password, gender, equipment, recolours, DEFAULT_SKILL_LEVELS, 0L);
    }

    public Profile(String username, String password, int gender, int[] equipment, int[] recolours, int[] skillLevels, long lastLogin) {
    ...
    public void drawProfileHead(int x, int y) {
        if (!username.isEmpty()) {
            final String imagePath = PROFILES_DIR + "/" + username.toLowerCase() + ".png";
    ...
    public Model getBodyModel() {
        Model[] bodyModels = new Model[equipment.length];
        int bodyModelsOffset = 0;
        for (int appearanceId : equipment) {
            if (appearanceId >= 256 && appearanceId < 512) {
                final IdentityKit kit = IdentityKit.kits[appearanceId - 256];
                Model subModel = kit.headModel();
                if (subModel != null) {
                    bodyModels[bodyModelsOffset++] = subModel;
                }
            }
            if (appearanceId >= 512) {
                ItemDefinition itemDef = ItemDefinition.lookup(appearanceId - 512);
        ...
        Model bodyModel = new Model(bodyModelsOffset, bodyModels);
        for (int i = 0; i < 5; i++) {
            if (recolours[i] != 0) {
                bodyModel.recolor(Client.PLAYER_BODY_RECOLOURS[i][0], Client.PLAYER_BODY_RECOLOURS[i][recolours[i]]);
                if (i == 1) {
                    bodyModel.recolor(Client.anIntArray1204[0], Client.anIntArray1204[recolours[i]]);
                }
            }
        }
        bodyModel.light(84, 850, -30, -50, -30, true);
        return bodyModel;
    }

    public Sprite convertBodyModelToSprite(Model model) {
        return ProfilePreviewRenderer.render(model);
    }
